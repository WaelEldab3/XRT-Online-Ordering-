...
export class UpdateItemUseCase {
            ...
            for (const override of mg.modifier_overrides) {
                ...
                if (override.prices_by_size && Array.isArray(override.prices_by_size)) {
                    ...
                    if (sizeCodes.length !== uniqueSizeCodes.size) {
                        throw new ValidationError('Duplicate size codes in prices_by_size for modifier override');
                    }

                    const validSizes = ['S', 'M', 'L', 'XL', 'XXL'];
                    for (const sizeCode of sizeCodes) {
                        if (!validSizes.includes(sizeCode)) {
                            throw new ValidationError(`Invalid size code: ${sizeCode}. Must be one of: S, M, L, XL, XXL`);
                        }
                    }
                ...
                if (override.quantity_levels && Array.isArray(override.quantity_levels)) {
                    const defaultCount = override.quantity_levels.filter((ql: any) => ql.is_default === true).length;
                    if (defaultCount > 1) {
                        throw new ValidationError('Only one quantity level can be set as default in modifier override');
                    }
                ...
                if (override.max_quantity !== undefined && (typeof override.max_quantity !== 'number' || override.max_quantity < 1)) {
    ...
    async execute(
        ...
        if (isSizeable) {
            ...
            if (itemData.is_sizeable === false && itemData.base_price === undefined && !existingItem.base_price && existingItem.base_price !== 0) {
