...
import { Permission, CreatePermissionDTO, UpdatePermissionDTO, PermissionFilters } from '../../domain/entities/Permission';
    ...
    async upsertMany(data: CreatePermissionDTO[]): Promise<{ inserted: number; skipped: number }> {
        ...
        for (const item of data) {
            ...
            if (!exists) {
                ...
                inserted++;
                console.log(`  ➕ New permission registered: ${item.key}`);
            } else {
                skipped++;
            }
        }
