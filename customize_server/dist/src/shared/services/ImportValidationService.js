...
class ImportValidationService {
     ...
     */
    static validate(data, business_id, filename = 'import.csv') {
        const errors = [];
        const warnings = [];
        // Validate Items
        ...
        // Validate Item Sizes
        const sizeKeys = new Map(); // item_key -> Set<size_code>
        const defaultSizes = new Map(); // item_key -> count of default sizes
        ...
        // Validate Modifiers
        const modifierKeys = new Map(); // group_key -> Set<modifier_key>
        ...
        data.itemModifierOverrides.forEach((override, index) => {
            ...
            if (!override.modifier_key) {
                ...
                const modifierExists = data.modifiers.some(m => m.modifier_key === override.modifier_key && m.group_key === override.group_key);
                if (!modifierExists) {
                    errors.push({
                        file: filename,
                        row,
