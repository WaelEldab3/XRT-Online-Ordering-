...
exports.RollbackImportUseCase = void 0;
...
class RollbackImportUseCase {
    ...
    async execute(sessionId, user_id) {
        ...
        if (session.status !== 'confirmed') {
            throw new AppError_1.ValidationError('Only confirmed sessions can be rolled back');
        }
        if (!session.rollbackData || session.rollbackData.length === 0) {
            throw new AppError_1.ValidationError('No rollback data found for this session');
        }
    ...
    async deleteEntity(entityType, id, business_id) {
        switch (entityType) {
            case 'category':
    ...
    async updateEntity(entityType, id, data, business_id) {
        switch (entityType) {
            case 'category':
...
exports.RollbackImportUseCase = RollbackImportUseCase;
