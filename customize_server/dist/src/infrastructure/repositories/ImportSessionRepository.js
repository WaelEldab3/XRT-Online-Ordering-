...
class ImportSessionRepository {
    ...
    async findByUser(user_id, business_id) {
        ...
        const sessionDocs = await ImportSessionModel_1.ImportSessionModel.find(query).sort({ created_at: -1 });
        return sessionDocs.map((doc) => this.toDomain(doc));
    }
    async update(id, user_id, data) {
        const sessionDoc = await ImportSessionModel_1.ImportSessionModel.findOneAndUpdate({ _id: id, user_id }, data, {
            new: true,
            runValidators: true,
        });
        if (!sessionDoc) {
    ...
    async deleteAll(user_id, business_id) {
        const query = { user_id };
        if (business_id) {
            query.business_id = business_id;
        }
        await ImportSessionModel_1.ImportSessionModel.deleteMany(query);
    }
    async deleteExpired() {
