...
const migrateItemSizes = async () => {
    try {
        ...
        const itemsWithSizes = await ItemModel_1.ItemModel.find({
        ...
        for (const item of itemsWithSizes) {
            try {
                ...
                const embeddedSizes = item.sizes || [];
                ...
                for (let i = 0; i < embeddedSizes.length; i++) {
                    ...
                    let uniqueCode = code;
                    let suffix = 1;
                    while (await ItemSizeModel_1.ItemSizeModel.exists({ item_id: itemId, code: uniqueCode })) {
                        uniqueCode = `${code}_${suffix}`;
                        suffix++;
                    }
                    const itemSize = new ItemSizeModel_1.ItemSizeModel({
                        item_id: itemId,
                ...
                await ItemModel_1.ItemModel.updateOne({ _id: itemId }, {
                    $unset: { sizes: '' },
                    $set: {
...
exports.default = migrateItemSizes;
