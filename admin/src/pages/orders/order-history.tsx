...
export default function OrderHistoryPage() {
  ...
  const { transactions, loading, paginatorInfo, error } = useTransactionsQuery({
    limit: 20,
    page,
    date,
  });
    ...
    <>
      <Card className="mb-8 flex flex-col items-center justify-between md:flex-row">
        <div className="mb-4 md:mb-0 md:w-1/4">
          <PageHeading title={t('form:form-title-order-history')} />
        </div>

        <div className="flex w-full items-center md:w-1/2 gap-4">
          <input
            type="date"
            className="h-12 border border-border-base rounded px-4 text-sm focus:border-accent focus:outline-none"
            value={date}
...
OrderHistoryPage.authenticate = {
  permissions: adminOnly,
};
OrderHistoryPage.Layout = Layout;
